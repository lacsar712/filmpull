...
type App struct {
	...
	speedMPM       float64
	speedRamp      *SpeedRamp
	lineSched      *clock.LineScheduler
	setWindow      *clock.SetWindow
	gripGuard      *interlock.GripGuard
	gripLeases     *interlock.GripLeaseRegistry
	stretchMu      sync.Mutex
	stretchCancels map[model.LineID]context.CancelFunc
}
...
func New(cfg config.Config) (*App, error) {
	...
	a.speedRamp = NewSpeedRamp(a.clk, cfg.SpeedRampSteps, cfg.ProcessTick())
	a.lineSched = clock.NewLineScheduler(*a.clk)
	a.setWindow = clock.NewSetWindow(a.clk, time.Duration(cfg.SetWindowMin)*time.Minute)
	a.gripGuard = interlock.NewGripGuard(15.0)
	a.gripLeases = interlock.NewGripLeaseRegistry(30 * time.Second)
	a.stretchCancels = make(map[model.LineID]context.CancelFunc)
	a.stats.IncEvent("grip_alarm_ready")
	a.persistSnapshot()
	return a, nil
}
